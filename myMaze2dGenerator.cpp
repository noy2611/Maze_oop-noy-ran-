...
Maze2d myMaze2dGenerator::generate(int M, int N, Maze2d maze)
{
    ...
    while (nVisited < m * n)
    {
        ...
        if (m_stack.top().second.first > 1)
        {
            ...
            if (maze.getCell(m_stack.top().second.first - 2, m_stack.top().second.second) == '#' &&
                !visited[maze.getCellIndex(m_stack.top().second.first - 2, m_stack.top().second.second)])
            {
        ...
        // East, South, West: Similar neighbor checks as above...
        ...
        if (!neighbours.empty())
        {
            ...
            // Create a path between this cell and neighbor
            ...
            case 0: // North
                ...
                m_stack.push(cell_list[maze.getCellIndex(m_stack.top().second.first - 2, m_stack.top().second.second + 0)]);
                break;
                // ... cases for other directions ...
            }
